...
### D) 정확도 향상 방법론 옵션 (16종 토글) ★핵심 요구
검증 품질의 정확도를 높이는 기법을 평가 시 선택. (이미 구현된 `EvaluationOptions`)
| 옵션 | 효과 |
|------|------|
| **cot** | 근거→점수 단계 추론으로 점수 선결정 편향 차단 (기본 ON) |
| **reverse** | 순방향+역방향 양방향 평가 → 환각 누락 탐지 |
| **correctness** ①② | 정답 대비 claim F1+유사도(정답성) + answer_relevance 정답 주입 채점 |
| **context_injection** ③ | 직전 N턴 맥락 주입 → 후속 턴 오채점 제거 (비용 0) |
| **abstention** ④ | 정당한 거절/모름 감점 면제 (거짓 실패 제거) |
| **numeric_guard** ⑤ | 숫자·금액·날짜 결정적 대조로 수치 환각 포착 (LLM 0) |
| **auto_calibration** ⑦ | 골든셋 보정맵 + SLA 임계 자동 튜닝 |
...
- **New Run 마법사** (스텝):
  ...
  4. **메트릭**: 레벨별 체크박스
  5. **정확도 방법론**: 16종 토글(§2.5 D) — 또는 프리셋 선택
